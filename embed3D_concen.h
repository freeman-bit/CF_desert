...
double embed_interpolate2 (Point point, scalar s, coord p)
{
  ...
  int k = sign(p.z);
    if (cs[i,0,0] && cs[0,j,0] && cs[i,j,0] &&
      cs[0,0,k] && cs[i,0,k] && cs[0,j,0] && cs[i,j,k]) {
    double val_0, val_k;
...
double embed_interpolate2 (Point point, scalar s, coord p)
{
  ...
  if (cs[i,0,0] && cs[0,j,0] && cs[i,j,0]&&cs[0,0,k] && cs[i,0,k] && cs[0,j,k] && cs[i,j,k])
    // 三线性插值 interpolation when all neighbors are defined
  return  (s[]*(1-fabs(p.x))*(1-fabs(p.y))*(1-fabs(p.z))+s[i,0,0]*fabs(p.x)*(1-fabs(p.y))*(1-fabs(p.z))+s[i,j,0]*fabs(p.x)*fabs(p.y)*(1-fabs(p.z))+s[0,j,0]*fabs(p.y)*(1-fabs(p.x))*(1-fabs(p.z))
+s[0,0,k]*(1-fabs(p.x))*(1-fabs(p.y))*fabs(p.z)+s[i,0,k]*fabs(p.x)*(1-fabs(p.y))*fabs(p.z)+s[i,j,k]*fabs(p.x)*fabs(p.y)*fabs(p.z)+s[0,j,k]*fabs(p.y)*(1-fabs(p.x))*fabs(p.z));
    
    else if (cs[i,0,0] && cs[0,j,0] && cs[i,j,0]&&cs[0,0,-k] && cs[i,0,-k] && cs[0,j,-k] && cs[i,j,-k])
    return  (s[]*(1-fabs(p.x))*(1-fabs(p.y))*(1-fabs(p.z))+s[i,0,0]*fabs(p.x)*(1-fabs(p.y))*(1-fabs(p.z))+s[i,j,0]*fabs(p.x)*fabs(p.y)*(1-fabs(p.z))+s[0,j,0]*fabs(p.y)*(1-fabs(p.x))*(1-fabs(p.z))
+s[0,0,-k]*(1-fabs(p.x))*(1-fabs(p.y))*fabs(p.z)+s[i,0,-k]*fabs(p.x)*(1-fabs(p.y))*fabs(p.z)+s[i,j,-k]*fabs(p.x)*fabs(p.y)*fabs(p.z)+s[0,j,-k]*fabs(p.y)*(1-fabs(p.x))*fabs(p.z));
...
foreach_dimension()
static inline double dirichlet_gradient_phi_x (Point point, scalar s, scalar s2, scalar cs, scalar css,
					face vector fs, face vector fss,
					   coord n, coord p, double bc,
					   double * coef, int flag)
{
  foreach_dimension()
    n.x = - n.x;
  double d[2], v[2] = {nodata,nodata};
  ...
  if (defined){
	...
	break;
      ...
      foreach_dimension()
      nn.x=-n.x;
      double d1=distance1 (point, cs, fs, nn, p) ;
      double d2=distance2 (point, cs, fs, nn, p) ;
      double d3 = distance1 (point,css,fss, n, p);
      double d4 = distance2 (point,css,fss, n, p); 
      
      double K1=d2/d1/(d2-d1);
      double K2=d1/d2/(d2-d1);
      double K3=d4/d3/(d4-d3);
      double K4=d3/d4/(d4-d3);
      
      
      double phi1=embedded_velocity1_normal (point, s, cs, fs, nn, p) ; 
      double phi2=embedded_velocity2_normal (point, s, cs, fs, nn, p) ; 
      double phi3=embedded_velocity1_normal (point, s2, css, fss, n, p) ; 
      double phi4=embedded_velocity2_normal (point, s2, css, fss, n, p) ; 
    
    
    if (flag==1)
     bc=(-(K2*phi2-K1*phi1)-SigR*(K4*phi4-K3*phi3))/(K1-K2+SigR*kp*(K3-K4)+SEPS);
     
    if (flag==0)
     bc=(-(K2*phi2-K1*phi1)-SigR*(K4*phi4-K3*phi3))/(K1-K2+SigR*(K3-K4)/kp+SEPS);
      
//  bc=(phi2/d2+phi4/d4)/(1/d2+1/d4);
//bc=(phi1/d1+phi3/d3)/(1/d1+1/d3);
  //  bc=0.;
    }
...
//    return 1.;  
 ...
 /////////////////////////////// 
  ...
  if (flag==1)
  bc=(phi1/d1+phi3/d3)/(1/d1+kp/d3);
  if (flag==0)
  bc=(phi1/d1+phi3/d3)/(1/d1+1/d3/kp);
...
double dirichlet_gradient_phi (Point point, scalar s, scalar s2, scalar cs, scalar css, 
				face vector fs, face vector fss,
			   coord n, coord p, double bc, double * coef, int flag)
{
#if dimension == 2
  foreach_dimension()
    if (fabs(n.x) >= fabs(n.y))
      return dirichlet_gradient_phi_x (point, s, s2, cs, css, fs, fss, n, p, bc, coef, flag);
#else // dimension == 3
  if (fabs(n.x) >= fabs(n.y)) {
    if (fabs(n.x) >= fabs(n.z))
      return dirichlet_gradient_phi_x (point, s, s2, cs, css, fs, fss, n, p, bc, coef, flag);
    ...
    return dirichlet_gradient_phi_y (point, s, s2, cs, css, fs, fss, n, p, bc, coef, flag);
  return dirichlet_gradient_phi_z (point, s, s2, cs, css, fs, fss, n, p, bc, coef, flag);
#endif // dimension == 3
  return nodata;
}
...
void embed_force2 (scalar p, vector u, face vector mu, coord * Fp, coord * Fmu)
{
  // fixme: this could be simplified considerably if reduction worked on vectors
  double Fpx = 0., Fpy = 0., Fpz = 0., Fmux = 0., Fmuy = 0., Fmuz = 0.;
  foreach (reduction(+:Fpx) reduction(+:Fpy) reduction(+:Fpz)
	   reduction(+:Fmux) reduction(+:Fmuy) reduction(+:Fmuz))
    if (cs[] > 0. && cs[] < 1) {
 coord n, b;
...
#if dimension >= 3
	Fmux -= area*mua*dudn.z*n.x*n.z;
	Fmuy -= area*mua*dudn.z*n.y*n.z;
	Fmuz -= area*mua*(dudn.z*(sq(n.z) + 1.) +
			  dudn.x*n.x*n.z +
			  dudn.y*n.y*n.z);
#endif
      }
...
double embed_vorticity3D (Point point, vector u, coord p, coord n)
{
 coord w;

  coord dudn = embed_gradient (point, u, p, n);
  w.x = dudn.z*n.y - dudn.y*n.z;
  w.y = dudn.x*n.z - dudn.z*n.x;
  w.z = dudn.y*n.x - dudn.x*n.y;

 return sqrt(sq(w.x)+sq(w.y)+sq(w.z));

}

double embed_vorticity3D_x (Point point, vector u, coord p, coord n)
{
  /**
  We compute $\mathbf{{\nabla}}\mathbf{u}\cdot\mathbf{n}$, taking
  the boundary conditions into account. */
...
double embed_flux_phi (Point point, scalar s, scalar s2, scalar cs, scalar css,
face vector fs, face vector fss, face vector mu, double * val, int flag)
{
...
//  if (dirichlet) {
    normalize (&n);
    grad = dirichlet_gradient_phi (point, s, s2, cs,css, fs, fss, n, p, grad, &coef, flag);
//  }
